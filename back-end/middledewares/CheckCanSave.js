...
const checkCanSave = async (req, res, next) => {
  try {
    ...
    // Cac truong hop pending,block.... khong duoc save
    if (post.status !== 1)
      return res.status(403).json({ message: "Post is not approved" });
    //pubic ai cung co the save
    if (post.privacy === 0) return next();
    //private chi co chu post moi co quyen save
    const user_id = post.user_id;
    const myId = req.user.userId;
    if (post.privacy === 2)
      return res.status(403).json({ message: "Cannot save private post" });
    // friend thi cho co friendship moi save duoc
    if (post.privacy === 1) {
    ...
    console.error("checkCansave error:", error);
